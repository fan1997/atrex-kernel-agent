...
def _submit(parser: argparse.ArgumentParser, args: argparse.Namespace) -> int:
    ...
    if args.wait_mode == "suspend" and args.agent_cli not in {"claude", "codex"}:
        parser.error(
            f"{args.agent_cli} cannot resume a suspended evaluation; use --wait-mode inline"
        )
        ...
        json.dumps(
            {
                ...
                "next_action": "end this Agent invocation; the supervisor will resume it",
            },
            indent=2,
        ),
        flush=True,
...
def _profile(parser: argparse.ArgumentParser, args: argparse.Namespace) -> int:
    ...
    if args.wait_mode == "suspend" and args.agent_cli not in {"claude", "codex"}:
        parser.error(
            f"{args.agent_cli} cannot resume a suspended profile; use --wait-mode inline"
        )
    ...
    if args.wait_mode == "inline":
        ...
        return 0
    write_evaluation_handoff(workspace, pending)
    print(
        json.dumps(
            {
                "status": "submitted",
...
def main(argv: list[str] | None = None) -> int:
    ...
    submit.add_argument("--wait-mode", choices=("inline", "suspend"), default="suspend")
    ...
    profile = subparsers.add_parser("profile", help="optional typed NCU/rocprof profile")
    ...
    profile.add_argument("--wait-mode", choices=("inline", "suspend"), default="inline")
    ...
    profile.add_argument("--route", choices=("auto", "typed", "repository"), default="auto")
