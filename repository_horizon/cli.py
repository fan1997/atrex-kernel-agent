...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Autonomous repository-backed Long Horizon optimizer"
    )
    ...
    parser.add_argument("--optimization-mode", choices=("production",), default="production")
    ...
    parser.add_argument("--episode-policy", choices=("autonomous",), default="autonomous")
    parser.add_argument("--max-iters", "--max-episodes", dest="max_episodes", type=int, default=20)
    parser.add_argument("--token-budget", type=int, default=0)
    parser.add_argument("--max-stall", type=int, default=0)
    parser.add_argument("--iter-timeout", type=int, default=18_000)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.sandbox_profile and args.sandbox_url:
            raise ValueError("--sandbox-profile and --sandbox-url are mutually exclusive")
        ...
        if 2 * args.verify_repeats * args.verify_run_timeout + 30 > args.sandbox_timeout:
            raise ValueError(
        ...
        if not (source_checkout / ".git").exists():
            raise ValueError(f"source checkout is not a Git checkout: {source_checkout}")
