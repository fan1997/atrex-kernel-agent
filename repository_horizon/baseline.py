...
from long_horizon.campaign import (
    _candidate_shape_latencies,
    _representative_candidate_result,
)
...
class RepositoryBaselineManager:
    ...
    def _ensure_measured_v0(self, campaign: Any) -> None:
        ...
        current_version = latest_version(campaign.workspace)
        current_memory = (
            read_memory(campaign.workspace, current_version)
            if current_version >= 0
            else None
        )
        if (
            isinstance(current_memory, dict)
            and (current_memory.get("quality_gate") or {}).get("result") == "PASS"
        ):
            return
        ...
        if (
            v0_path.is_file()
            and (memory.get("correctness") or {}).get("status") == "PASS"
        ):
            return
        if (
            bringup_enabled
        ...
        verifier = RepositoryABBAValidator(
            ...
            private_reference_dir=getattr(campaign, "private_reference_dir", None),
        )
        ...
        v0_memory["performance"] = {
            ...
            "shape_ids_are_opaque": getattr(campaign, "private_reference_dir", None)
            is not None,
