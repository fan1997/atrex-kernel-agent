...
def write_agent_result(
    ...
    *,
    ...
    if len(encoded) > max_bytes:
        ...
        payload["profile"] = {"summary": "profile detail omitted; inspect evidence selectively"}
        encoded = json.dumps(payload, ensure_ascii=False, indent=2).encode("utf-8")
    if len(encoded) > max_bytes:
...
def compact_resume_prompt(path: Path, max_bytes: int = 2 * 1024) -> str:
    value = json.loads(path.read_text(encoding="utf-8"))
    prompt = (
        f"Evaluation {value.get('evaluation_id')} completed: "
        f"{value.get('classification')}; improvement_pct={value.get('improvement_pct')}. "
        f"Read the compact result at `{path}`. Inspect deeper evidence only when it supports the "
        "current hypothesis. Do not resubmit the same request digest. Continue this same episode."
    )
    encoded = prompt.encode("utf-8")
    if len(encoded) > max_bytes:
        prompt = encoded[:max_bytes].decode("utf-8", errors="ignore")
    return prompt
