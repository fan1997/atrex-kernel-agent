...
def _build_repository_stage(
    ...
    *,
    ...
    shutil.copy2(Path(__file__).with_name("repository_profile.py"), stage / "repository_profile.py")
    tools = stage / "tools"
    tools.mkdir()
        ...
        stage / "profile_request.json",
        {
            **detail,
            ...
            "kernel_filter": (
                ...
                or (f"regex:{detail['kernel_regex']}" if detail.get("kernel_regex") else "")
            ),
        },
    )
...
def submit_profile(
    ...
    *,
    ...
    if kernel_regex and not (kernel_regex.startswith("^") and kernel_regex.endswith("$")):
    ...
    directory = workspace / ".repository_horizon_runtime" / "evaluations" / evaluation_id
    ...
    if route == "auto":
        ...
        route = (
            ...
            and all((reference_dir / name).is_file() for name in ("reference.py", "input.py", "shapes.json"))
            else "repository"
        )
        detail["route"] = route
...
def collect_profile(pending: PendingVerification) -> VerificationResult:
    ...
    result = VerificationResult(
        ...
        error="" if ok else str(
            payload.get("error") or response.get("error") or f"profile status={snapshot.status}"
        ),
