...
def _promisor_remote(source: Path) -> str | None:
    ...
    for remote in remotes:
        ...
        if promisor.returncode != 0 or promisor.stdout.strip() != "true":
            continue
        url = _git(source, "remote", "get-url", remote, check=False).stdout.strip()
        if url:
            return url
    return None


def _fetch_commit(source: Path, destination: Path, commit: str, local_name: str) -> str:
...
def build_source_corpus(
    ...
    if config.mode == "allowlist":
        requested.extend(
            (f"ref_{index:04d}", ref) for index, ref in enumerate(config.refs)
        )
    ...
    commits = _bare(
        destination, "rev-list", "--all", "--topo-order"
    ).stdout.splitlines()
    ...
    if config.mode == "replay_strict":
        ...
        ancestors = set(_bare(destination, "rev-list", revision).stdout.splitlines())
...
def validate_source_corpus(
    ...
    *,
    ...
    if expected_runtime is not None:
        try:
            if corpus.resolve() != expected_runtime.resolve():
                violations.append(
                    "episode source corpus points outside campaign runtime"
                )
        except OSError:
    ...
    if len(commit_lines) != catalog.get("commit_count") or _digest_lines(
        sorted(commit_lines)
    ) != catalog.get("commit_set_sha256"):
        violations.append("source corpus reachable commit set changed")
    objects = _object_inventory(corpus)
    if len(objects) != catalog.get("object_count") or _digest_lines(
        objects
    ) != catalog.get("object_set_sha256"):
    ...
    for excluded in catalog.get("excluded_commits") or []:
        ...
        if present.returncode == 0:
            violations.append(
                f"excluded commit is present in source corpus: {excluded}"
            )
    return violations
...
def corpus_has_path(workspace: Path, commit: str, path: str) -> bool:
    corpus = workspace / CORPUS_RELATIVE
    return (
        _bare(corpus, "cat-file", "-e", f"{commit}:{path}", check=False).returncode == 0
    )
